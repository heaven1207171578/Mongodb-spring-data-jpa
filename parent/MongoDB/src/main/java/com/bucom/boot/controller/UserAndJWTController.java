...
public class UserAndJWTController {
  ...
  @Autowired private HttpServletRequest request; // 获取请求头部
  ...
  public ResultData login(@RequestParam Map<String, Object> map) {
    ...
    /** 第一个参数为前端传来的值 第二个参数数据库查的密码加密值 */
    boolean matches = encoder.matches(pwd, byName.getPassWord());
    if (matches) {
      String token =
          jwtUtils.createJWT(byName.getId(), byName.getUserName(), (String) byName.getRoles());
  ...
  public ResultData findUserAndLimit() {
    ...
    /** ^.*张$ :右匹配 ^张.*$ :左匹配 ^.*张.*$ :模糊匹配 Pattern.CASE_INSENSITIVE:不区分大小写 */
  ...
  public Page<User> findUserPage(@RequestBody Map<String, Object> map, Pageable pageable1) {
    ...
    Page<User> byUserNameLike = userRepository.findByUserNameLike(userName, pageable1);
